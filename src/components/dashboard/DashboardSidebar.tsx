...
export default function DashboardSidebar() {
  ...
  const sidebarContent = (
    <div
      className={`flex flex-col h-full bg-slate-900 dark:bg-slate-950 text-slate-300 select-none border-r border-slate-800 transition-all duration-300 ease-in-out ${
        isCollapsed ? 'w-20' : 'w-64'
      }`}
    >
      {/* Brand Header */}
      <div
        className={`h-16 px-4 flex items-center justify-between border-b border-slate-800 bg-slate-950/80 ${
        ...
        <div className="pt-4">
          {!isCollapsed ? (
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider hover:text-slate-200 transition-colors group"
            >
          ...
          {(isDatabaseOpen || isCollapsed) && (
            <div className={isCollapsed ? 'space-y-1' : 'mt-1 ml-3 pl-3 space-y-1 border-l border-slate-800'}>
              {databaseSubItems.map((sub) => {
                ...
                return (
                  ...
                  >
                    <span className={subActive ? 'text-amber-300' : 'text-slate-400'}>{sub.icon}</span>
      ...
      <div className="p-3 border-t border-slate-800 bg-slate-950/60">
        <Link
          ...
          } text-xs font-semibold text-slate-400 hover:text-white rounded-lg hover:bg-slate-800/50 transition-colors`}
        >
