            ...
            <tbody className="divide-y divide-slate-100">
              {jabatanList.map((j) => (
                <tr key={j.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="py-4 px-4 font-bold text-slate-900">{j.namaJabatan}</td>
                  <td className="py-4 px-4 font-semibold">
                    <span className="px-2.5 py-1 text-[10px] font-extrabold rounded-md bg-slate-100 text-slate-700 border border-slate-200">
                      {j.tingkat}
                    </span>
                  </td>
                  <td className="py-4 px-4 text-slate-500 max-w-xs">{j.deskripsi}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs">
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-extrabold text-slate-900">Tambah Jabatan Baru</h3>
              <button
                type="button"
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Kode Jabatan</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Tingkat Struktur</label>
                <select
                  value={newTingkat}
                  onChange={(e) => setNewTingkat(e.target.value as any)}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg font-semibold outline-none"
                >
